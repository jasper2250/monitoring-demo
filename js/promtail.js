        ...
        if(target.type === 'azure-vm-scale-set'){
            ...
            vmssNames.forEach(vmssName => {
                getVmssIps(vmssName, resourceGroup, subscriptionId, credentials).then(vmssIps => {
                    vmssIps.forEach(vmssIp => {
                        shell.mkdir('-p', 'config/'.concat(stageName).concat('/').concat(vmssIp));
                        let config=jsyaml.dump(promtail);
                        fs.writeFileSync('config/'.concat(stageName).concat('/').concat(vmssIp).concat('/promtail-config.yml'),config);
            ...
            vmNames.forEach(vmName => {
                getVmIp(vmName, resourceGroup, subscriptionId, credentials).then(result => {
                    shell.mkdir('-p', 'config/'.concat(stageName).concat('/').concat(result));
                    let config=jsyaml.dump(promtail);
                    fs.writeFileSync('config/'.concat(stageName).concat('/').concat(result).concat('/promtail-config.yml'),config);
            ...
            hosts.forEach(host => {
                shell.mkdir('-p', 'config/'.concat(stageName).concat('/').concat(host));
                let config=jsyaml.dump(promtail);
                fs.writeFileSync('config/'.concat(stageName).concat('/').concat(host).concat('/promtail-config.yml'),config);
